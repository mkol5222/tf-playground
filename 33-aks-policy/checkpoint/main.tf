...
resource "azurerm_subnet" "cpfront-subnet" {
  name                 = "cpfront-subnet"
  virtual_network_name = var.virtual_network_name
  resource_group_name  = var.vnet_rg.name
  address_prefixes     = ["10.68.10.0/24"]
}

resource "azurerm_subnet" "cpback-subnet" {
  name                 = "cpback-subnet"
  virtual_network_name = var.virtual_network_name
  resource_group_name  = var.vnet_rg.name
  address_prefixes     = ["10.68.11.0/24"]
}
...
resource "azurerm_network_interface" "nic1" {
  ...
  ip_configuration {
    name                          = "ipconfig1"
    subnet_id                     = azurerm_subnet.cpfront-subnet.id
    private_ip_address_allocation = var.vnet_allocation_method
    private_ip_address            = "10.68.10.4" //cidrhost(var.subnet_prefixes[0], 4)
...
resource "azurerm_network_interface" "nic2" {
  ...
  ip_configuration {
    name                          = "ipconfig2"
    subnet_id                     = azurerm_subnet.cpback-subnet.id
    private_ip_address_allocation = var.vnet_allocation_method
    private_ip_address            = "10.68.11.4" //cidrhost(azurerm_subnet.cp-back.subnet_prefixes[0], 4)
  }
  lifecycle {
    ignore_changes = [
      # Ignore changes to tags, e.g. because a management agent
...
locals {
  initscript = templatefile("${path.module}/cloud-init.sh", {
      ...
      enable_custom_metrics         = "no" #var.enable_custom_metrics ? "yes" : "no"
    })
}

output "initscript" {
  value = local.initscript
}
